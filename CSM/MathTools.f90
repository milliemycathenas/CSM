...
subroutine DpOrder(DIM,energy,basis,state)
!order double precision energies
	...
	double precision :: energy_tmp
	real, dimension(10) :: basis_tmp
	real, dimension(DIM) :: state_tmp

	double precision, dimension(DIM) :: energy
	real, dimension(DIM,10) :: basis
	real, dimension(DIM,DIM) :: state  
...
end subroutine DpOrder

subroutine OrderCmplx(DIM,energy,basis,state)
	...
	real, dimension(10) :: basis_tmp
	real :: energy_tmp
	complex, dimension(DIM) :: state_tmp

	real, dimension(DIM,10) :: basis
	real, dimension(DIM) :: energy
	complex, dimension(DIM,DIM) :: state  

	do i=1, DIM-1
	do j=1, DIM-i
		if (abs(energy(j)) > abs(energy(j+1))) then
			energy_tmp=energy(j)
			energy(j)=energy(j+1)
			energy(j+1)=energy_tmp
